...
class TraceExpr(object):
    ...
    def replace_with_recurisve_expr(self, subvariable, recurisve_expr, var_type=None):
        ...
        new_expr = self.replace(subvariable, replacement, re_sim_actions, rep_type=var_type, rep_info=rep_info, base_ptr=base_ptr)
    ...
    def make_backward_copy(self):
        ...
        new_trace_expr.forward_path = self.forward_path
        new_trace_expr.backward_path = self.backward_path
        new_trace_expr.cycle_locs = self.cycle_locs
        new_trace_expr.guard = self.guard
...
class VarExpr(object):
    ...
    def __init__(self, ast, value=None, pattern=None, trace_dir=None, data_type=None, var_type=None, base_ptr=None):
        ...
        # trace_vars: the variable (r or t) should be traced.
        # killed_vars: the variable (r) that have been redefined.
        # self.trace_vars = set()
        # self.killed_vars = set()

        self.pattern = pattern

        # The trace data type, include Vptr/Iptr/Fptr/Dptr/Ret etc.
        self.data_type = data_type

        # THe trace variable expression's type. (int, char, ptr, etc.)
        self.var_type=var_type

        # Lable the variable expr trace by forward or backward.
        self.trace_dir = trace_dir

        # Record the variable update path.
        self.store_location = []

        # Record the store block addr in trace.
        # self.store_addrs = []

        # A unique id for the expr.
        self.alias_id = 0
        self.contain_alias = set()

        self.ptr_id = 0

        # Taint ID label the taint context id.
        self.taint_id = 0

        # could be removed!!!
        # self.ls_idx = None

        # The trace expr's source location.
        self.source = None
        ...
        # Label the breaked off backward exprs.
        # self.bw_loc = None
    ...
    def copy(self):
        ...
        # new_expr.trace_vars = self.trace_vars.copy()
        # new_expr.killed_vars = self.killed_vars.copy()
    ...
    def contain_argument_or_global(self, arguments=None):
        """
        Whether expr contain argument or global pointer?
        """
        for var, sim in self.sims.items():
    ...
    def contain_argument_or_global_bak(self, arguments=None):
        """
        Whether expr contain argument or global pointer?
        """
        for var, sim in self.sims.items():
            if ((sim.var_type == 'ptr' or sim.var_type is None) and
                    (type(var) is str and is_arg_symbol(var, arguments) or
                     type(var) is int and get_scope(var) == 'global')):
                return True
        return False
    ...
    def is_global(self, arguments=None):

        for var, sim in self.sims.items():
            if type(var) is int and get_scope(var) == 'global':
                g_ptr = True

            elif type(var) is str and is_arg_symbol(var, arguments) and sim.var_type == 'ptr':
                a_ptr = True

        if g_ptr and not a_ptr:
            return 'G'

        elif a_ptr and not g_ptr:
            return 'A'

        for i, sim_action in self.sim_actions.items():
            if sim_action.var_type != 'ptr':
                pass
...
class RecursiveExpr(TraceExpr):
    ...
    def replace(self, subvariable, replacement, re_sim_actions=None, sub_type=None, rep_type=None, rep_info=None):
        ...
        if new_base is not None:
            ...
            l.info("Not find new base in new expr %s" % (new_expr))
            new_recursive_expr.base = self.base
        new_recursive_expr.offset = self.offset
        new_recursive_expr.position = self.position
    ...
    def make_backward_copy(self):
        new_expr = self.expr.make_backward_copy()
        return RecursiveExpr(new_expr, index=self.index, base=self.base, offset=self.offset, position=self.position)
