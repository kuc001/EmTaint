...
# collect Iptr data type info
collect_icall_exprs = defaultdict(list)
# collect Vptr data type info
collect_ptr_exprs = defaultdict(list)
# collect Fptr data type info
collect_fptr_exprs = defaultdict(list)
# collect use Arg
context_use_exprs = defaultdict(list)
# collect def Arg
context_def_exprs = defaultdict(list)
# collect Aptr
collect_aptr_exprs = defaultdict(list)
# collect function Ret
collect_ret_exprs = defaultdict(list)
# collect function return pointer Rptr
collect_rptr_exprs = defaultdict(list)
...
custom_sources = ['gcgiFetchString', 'get_cgi']
...
class FastSearch(BinaryInfo):
    def __init__(self, project, binary_parser,
        ...
        self.taint_source = ['BIO_read', 'recv', 'recvfrom', 'SSL_read', 'getenv', 'fgets', 'fread']
    ...
    def _test_taint(self, start_block):

        print("custome initial %s" % (start_block))
        ...
        # print("%s %s" % (trace_expr, trace_expr2))
    ...
    def _analyze_binary(self, do_recursive_call=True):
        ...
        for start_function in self._start_functions:
            ...
            worklist = []
            self._ida_object.cg.get_pre_sequence_call_graph(start_function, tree_nodes, worklist)
            # print("Analyze functions: %d" % (len(set(worklist))))

            for i in range(len(worklist)-1, -1, -1):
                function = worklist[i]

                # DEBUG
                print("Analyze %s" % (function))
                ...
                # print("psu-debug: %s %s" % (function, contain_source))
                ...
                if self.debug:
                    ...
                    continue
                    # if funcea == 0x18d44:
                    #     self._test_taint(function.start_node)
                    # else:
                    #     continue
    ...
    def _execute_function(self, function, callsite=None, collect_ret=False, execute_flag=0, taint_context_id=0):
        ...
        count = 1
        while True:
            print("="*55 + "analyze-function %x (%d - %x)" % (funcea, analyzed_count, function.flag) + '='*55)
        ...
        self._collect_analysis_results(function, execute_flag, taint_context_id)
        self._collect_result_in_exit_blocks(function, execute_flag, taint_context_id)
        ...
        for block in pre_sequence_nodes:
            ...
            if block.is_loop:
                ...
                for block in loop.body_nodes:
                    ...
                    if block.irsb:
                        self._accurate_dataflow.execute_block_irsb_v4(block, function_reg_defs, function_stack_defs, arguments)
    ...
    def _collect_analysis_results(self, function, execute_flag, taint_context_id=0):

        start_node = function.start_node
        funcea = function.addr

        print("Collect analysis results in function: %s (%d)" % (function, taint_context_id))
        for trace_expr in start_node.done_exprs:
            print("start node %s has: %s" % (start_node, trace_expr))
            ...
            if trace_expr.argument_or_global():
                ...
                if execute_flag == 0x2 and data_type == 'Tdata' and taint_context_id:
                    print("Should collect callee taint (%x): %s, is-ret: %s" % (funcea, trace_expr, trace_expr.expr.is_ret))
                    function.taint_exprs.append(trace_expr)
                    # function.taint_exprs[taint_context_id].append(trace_expr)
    ...
    def _collect_result_in_exit_blocks(self, function, execute_flag=0, taint_context_id=0):
        """
        ...
        for block in exit_blocks:
            ...
            for trace_expr in block.done_exprs:
                print("Ret-xxx: %s has %s" % (block, trace_expr))
                sims = trace_expr.expr.sims
                ast = trace_expr.expr.ast
                ...
                if len(sims) == 1 and self.ret_name in sims and sims[self.ret_name].live:
                    ...
                    elif data_type == 'Tdata' and execute_flag == 0x2 and taint_context_id:
    ...
    def _push_callee_arg_exprs_to_callsite(self, function):
        ...
        for use_expr in argument_use_exprs:
            print("%s push-use: %s" % (function, use_expr))
        ...
        for pre_function in pre_functions:
            ...
            for callsite in caller_sites:
                # print("psu-test: callsite: %s" % (callsite))
                self._make_def_copy_to_callsite(callsite, argument_def_exprs)
                print("push expr to callsite %s\n expr: %s" % (callsite, argument_def_exprs))
    ...
    def _push_callee_ret_exprs_to_callsite(self, function):
        ...
        for ret_expr in ret_def_exprs:
            print("push-ret: %s has %s" % (function, ret_expr))
        ...
        for pre_function in pre_functions:
            ...
            for callsite in caller_sites:
                ...
                print("push ret-expr to callsite %s\n expr: %s" % (callsite, ret_def_exprs))
    ...
    def _make_def_copy_to_callsite(self, callsite, argument_defines):
        """
        :param callsite: a block node which has a call.
        ...
        for expr in argument_defines:
            ...
            print("push-to-callsite: %s %x" % (new_expr, new_expr.expr.flag))
    ...
    def _initialize_ret_register(self, function, ret_type):
        exit_blocks = function.cfg.exit_blocks
        print("Exit blocks: %s" % (exit_blocks))
    ...
    def _get_argument_taint_info(self, function, callsite, call_type=None):
        """
        ...
        for trace_expr in forward_exprs:
            ...
            source = trace_expr.expr.source
            if source.block_addr == callsite.addr:
                continue
        ...
        print("Get taint in %s" % (function))
        print("taint info: %s" % (callsite_taint_exprs))
        return callsite_taint_exprs

    def _get_callsite_argument_pointer_info(self, callsite):
        """
        ...
        for trace_expr in forward_exprs:
            ...
            if data_ast.op in ['BVS', 'Store'] and data_type in ['Vptr', 'Fptr', 'Aptr']:
                for var, sim in trace_sims.items():
                    if sim.var_type == 'ptr' and type(var) == str:
                        if data_type == 'Fptr':
                            t_pattern = 'BF'
                        elif data_type == 'Vptr':
                            t_pattern = 'LBF'
    ...
    def _call_into_callee_backward(self, function, callsite, parameter_ptrs):
        """
        ...
        for arg in parameter_ptrs:
            if arg in function.arg_contexts or arg not in function.arguments:
                continue
            arg_ptr_info[arg] = info
            function.arg_contexts.append(arg)
        ...
        if len(arg_ptr_info):
            ...
            print("arg-ptrs: %s" % (arg_ptr_info))
            self._execute_function(function, execute_flag=0x3)

    def _call_into_lib_callee(self, function, callsite, libary_object, loop, flow_dir):
        """
        Call into lib function, this mode should have load libary binaries.
        """
        is_taint = True

        self._pre_process_lib_function(function, libary_object)
        start_block = function.start_node

        callsite_taint_exprs = self._get_argument_taint_info(function, callsite, call_type='lib')
        ...
        taint_context_id = 0
        self._check_args_in_exprs(function, callsite_taint_exprs)
        if len(callsite_taint_exprs):
            taint_context_id = function.calculate_context_id(callsite_taint_exprs)
        ...
        if is_taint:
            self._accurate_dataflow.set_concret_contexts(callsite, function)

            print("Call-lib: %s" % (function.procedural_name))
            if function.procedural_name not in ['hsearch_r', 'hdestroy_r', 'hcreate_r']:
                debug

            self._execute_function(function, execute_flag=0x2, taint_context_id=taint_context_id)

        if len(function.taint_exprs):
            remove_exprs = []
            for t_expr in function.taint_exprs:
                print("%s has %s" % (function, t_expr))

            callee_taint_exprs = function.get_taint_exprs(backup_taint_exprs)
            ...
            self._process_callsite_callee_taint_in_forward(callsite, loop, callee_taint_exprs, flow_dir)

    def _call_into_callee_forward(self, function, callsite, loop, flow_dir):
        """
        Call into callee function, and get the callee's argument definitions and alias.
        Or trace the taint expr by forward analysis.
        """
        is_taint, is_arg = True, True
        ...
        callsite_taint_exprs = self._get_argument_taint_info(function, callsite, call_type='main')

        arg_ptr_info = self._get_callsite_argument_pointer_info(callsite)
        ...
        # the callee function (data flow analysis).
        if len(arg_ptr_info):
            function.check_arg_ptrs(arg_ptr_info)
        ...
        self._check_args_in_exprs(function, callsite_taint_exprs)
        if len(callsite_taint_exprs):
            print("psu_debug: %s has taint:\n %s" % (function, callsite_taint_exprs))
            taint_context_id = function.calculate_context_id(callsite_taint_exprs)
            # print("xpx: %s" % (function.taint_contexts))
            taint_flag = function.is_taint_context_analyzed(callsite_taint_exprs)
            print("taint_flag: %s" % (taint_flag))
            if taint_flag:
                is_taint = False
        ...
        print("%s has Callsite taint: %s" % (callsite, callsite_taint_exprs))

        if len(function.taint_exprs):
            remove_exprs = []
            callee_taint_exprs = function.get_taint_exprs(callsite_taint_exprs)
            print("%s has taint exprs (main):\n %s" % (function, callee_taint_exprs))
            for taint_expr in callee_taint_exprs:
        ...
        if arg_flag:
            self._process_callsite_callee_aptr_in_forward(callsite, function, loop, flow_dir)

    def _process_callee_function_in_forward(self, callsite, function, loop, flow_dir):
        """
        According to the context info to execute the callee function.
        """
        do_ret = True

        function_name = function.procedural_name
        ...
        if function_name and function_name in self._sim_procedures and self.taint_check: # or is_hooked(funcea)
            ...
            # do_ret = proc.run(callsite, flow_dir)
            check_ret = proc.execute(callsite, self.proj, 'run', flow_dir=flow_dir)

        elif function_name in self.ignore_lib_functions:
            do_ret = True

        elif function.binary_name == 'main':
            self._call_into_callee_forward(function, callsite, loop, flow_dir)
            do_ret = False

        elif self.taint_check:
            libary_object = self._libary_objects.get(function.binary_name)
            if libary_object is None:
                l.info("The libary %s has not been analyzed by Ida Pro!!!" % (function.binary_name))
                return do_ret

            self._call_into_lib_callee(function, callsite, libary_object, loop, flow_dir)

        return do_ret

    def _process_callee_function_in_backward(self, callsite, function, parameter_ptrs, loop, flow_dir):
        """
        ...
        if function_name and function_name in self._sim_procedures and self.taint_check: # or is_hooked(funcea)
            proc = self._sim_procedures[function_name]
            check_ret = proc.execute(callsite, self.proj, 'run', flow_dir='B')

        elif function.binary_name == 'main':
            # Should analyze callee function to get real ret value and update.
            print("Update-ret, should analyze %s" % (function))
            self._update_ret_variable(callsite, function)

            # Should analyze callee function to get parameter ptrs' definitions.
            if len(parameter_ptrs):
                self._call_into_callee_backward(function, callsite, parameter_ptrs)

            self._process_callee_definiton_in_callsite(callsite, function, loop, flow_dir)
            check_ret = False

        return check_ret

    def _process_callsite_callee_aptr_in_forward(self, callsite, callee_func, loop, flow_dir):
        """
        In forward, process callee parameter_ptrs' definitions and alias.
        """
        target = callee_func.addr
        ...
        if len(aptr_aliases):
            self._process_arg_ptr_aliases_forward(callsite, aptr_aliases, loop, flow_dir)

    def _process_arg_ptr_aliases_forward(self, callsite, aptr_aliases, loop, flow_dir):
        """
        ...
        for trace_expr in callsite.forward_exprs:
            ...
            if base_ptr in reg_alias_info:
                ...
                if len(new_exprs):
                    print("In-C %s, get new-exprs:\n %s" % (callsite, new_exprs))

        # TODO load alias info update!!!

        self._push_backward_exprs_to_pre_block(callsite, loop, new_backward_exprs, flow_dir)

    def _process_callee_definiton_in_callsite(self, callsite, callee_func, loop, flow_dir):
        """
        In backward, process callee parameters definitions and argument pointer alias.
        """
        callee_addr = callee_func.addr
        aptr_exprs = self.collector.datas['Aptr'].get(callee_addr)
        if not aptr_exprs:
            return

        print("%s has aptr-exprs: \n%s" % (callsite, aptr_exprs))
        aptr_defs = self._get_aptr_definitions(aptr_exprs)
        aptr_aliases = self._get_aptr_aliases(aptr_exprs)

        if len(aptr_defs):
            self._process_arg_ptr_defs(callsite, aptr_defs, loop, flow_dir)

        if len(aptr_aliases):
            self._process_arg_ptr_aliases_backward(callsite, aptr_aliases, flow_dir)

    def _process_arg_ptr_defs(self, callsite, aptr_defs, loop, flow_dir):
        """
        ...
        for aptr_def in aptr_defs:
            ...
            new_def = self._update_parameter_with_context(context_block, aptr_def)
    ...
    def _update_expr_v2(self, alias_exprs, trace_expr):
        ...
        for alias_expr in alias_exprs:
            ...
            new_expr = trace_expr.replace(value, alias_ast, re_sim_actions=rep_sim_actions, rep_type='ptr')
            new_expr.expr.trace_dir = 'B'
    ...
    def _update_parameter_with_context(self, callsite, aptr_expr):
        ...
        for arg in value.variables:
            if arg in default_arg_names:
                arg_context = self._find_reg_context(arg, live_defs)
                if arg_context:
                    arg_ast = BVS(arg)
                    context_ast = self._accurate_dataflow._generate_ast(arg_context)
    ...
    def _find_reg_context(self, reg, live_defs):
        ...
        elif reg_at.value and reg_at.var_type == 'ptr':
            reg_context = reg_at.value
    ...
    def _process_callsite_callee_taint_in_forward(self, callsite, loop, taint_exprs, flow_dir):
        """
        ...
        if len(new_forward_exprs):
            self._push_forward_exprs_to_succ_block(callsite, loop, new_forward_exprs, flow_dir)
    ...
    def _check_trace_flag(self, callsite, loop, flow_dir):
        """
            ...
            for pre_block in callsite.predecessors:
                if len(pre_block.backward_exprs):
                    if pre_block not in cycle_nodes:
                        function_trace_record[callsite.func_addr] = 1
            ...
            for succ_block in callsite.successors:
                if len(succ_block.forward_exprs):
                    if succ_block not in cycle_nodes:
                        function_trace_record[callsite.func_addr] = 2

                    else:
                        loop.execute_flag = 0x2

    def _pre_execute_callsite(self, function, callsite, loop=None, flow_dir=None):
        """
        ...
        for pre_block in callsite.predecessors:
            ...
            self._block_execute(function, pre_block, loop, 'B')

    def _block_execute(self, function, block, loop=None, flow_dir=None):
        """
        ...
        if block.irsb is not None:
            trace_flag = self._block_trace2(block)

        print("Block %s has type %s" % (block, block.node_type))
        if block.node_type in ['Call', 'Extern', 'iCall']:
            ...
            if type(block.target) is str:
                ...
                if callee_function is None and lib_func_name in self._sim_procedures:
                    ...
                    if self.taint_check:
                        check_forward_ret = proc.execute(block, self.proj, 'run', flow_dir='F')
                    self._pre_execute_callsite(function, block, loop, flow_dir)

                    if len(block.backward_exprs):
                        check_backward_ret = proc.execute(block, self.proj, 'run', flow_dir='B')
            ...
            if callee_function:
                ...
                if len(block.forward_exprs) or (func_name and func_name in self._sim_procedures and self.taint_check):
                    check_forward_ret = self._process_callee_function_in_forward(block, callee_function, loop, flow_dir)
                    self._pre_execute_callsite(function, block, loop, flow_dir)

                if len(block.backward_exprs):
                    parameter_ptrs = predict_parameter_pointer_in_backward(block)
                    check_backward_ret = self._process_callee_function_in_backward(block, callee_function, parameter_ptrs, loop, flow_dir)

            if check_forward_ret:
                # Check the return default register in block.forward_exprs
                print("Should check return in callsite-ret: %s" % (block))
                self._check_return_register_redefine(block)
            ...
            self._check_trace_flag(block, loop, flow_dir)

        pre_backward_exprs = self._get_predecessor_block_backward_exprs(block)
        succ_forward_exprs = self._get_successor_block_forward_exprs(block)

        # DEBUG
        print("push backward exprs: %s" % (pre_backward_exprs))
        print("push forward exprs: %s" % (succ_forward_exprs))
        ...
        self._push_backward_exprs_to_pre_block(block, loop, pre_backward_exprs, flow_dir)

        self._push_forward_exprs_to_succ_block(block, loop, succ_forward_exprs, flow_dir, function)
    ...
    def _check_taint_flag(self, callsite, trace_exprs, flow_dir):
        ...
        for trace_expr in trace_exprs:
            ...
            if trace_expr.expr.data_type == 'Tdata' and trace_expr.expr.source.block_addr == callsite.addr:
                if flow_dir == 'F':
    ...
    def _block_trace2(self, block):

        flag = 0

        print("\nDEBUG execute block: %s\n" % (block))
        ...
        while True:
            ...
            if len(backward_exprs):
                print("call backward_data_trace!")
    ...
    def _push_forward_exprs_to_succ_block(self, block, loop, succ_forward_exprs, flow_dir, function=None):
        ...
        for trace_expr in succ_forward_exprs:
            ...
            if self.taint_check and ptr_id in block.taint_constraints:
                guard_var = block.taint_constraints[ptr_id]
                trace_expr.expr.constraints.append(guard_var)
        ...
        for succ_block in block.successors:
            ...
            # branch_falg = block.judge_pointer_guard_forward(succ_block, trace_ptrs)
            # print("Branch-flag-2: %s" % (branch_falg))
    ...
    def _push_backward_exprs_to_pre_block(self, block, loop, pre_backward_exprs, flow_dir):
        ...
        for pre_block in block.predecessors:
            ...
            # print("push pre_block (%s): %s\n exprs- %s" % (pre_block, flow_dir, pre_block.backward_exprs))
            # print("UUU - flow_dir: %s\n with %s" % (flow_dir, pre_block.backward_exprs))

            # if flow_dir == 'F' and len(pre_block.backward_exprs):
            if len(pre_block.backward_exprs):
                if pre_block not in cycle_nodes:
                    function_trace_record[block.func_addr] |= 1
                    print("Changed, should analyze backward again in %s" % (block))
    ...
    def _check_return_register_redefine(self, block):
        """
    ...
    def _get_successor_block_forward_exprs(self, block):
        """
        ...
        for expr in block.forward_exprs:
            ...
            # print("Get-succs: %s %s" % (block, expr))
    ...
    def _get_predecessor_block_backward_exprs(self, block):
        """
    ...
    def _merge_block_exprs(self, src, dst, new_exprs, cycle_nodes, flow_dir=None):
        ...
        if flow_dir == 'F':
            for expr in merge_exprs:
                ...
                # print("HHH(F)- %s -> %s %s" % (src, dst, expr_copy))
            ...
            for expr in merge_exprs:
                ...
                expr_copy.guard = None
                # print("HHH(B)- %s -> %s %s" % (src, dst, expr_copy))
                old_exprs.append(expr_copy)
    ...
    def _update_return(self, trace_expr, ret_exprs):
        ...
        for ret_expr in ret_exprs:
            ...
            if type(ret_expr) is RecursiveExpr:
                ...
                new_expr.expr.constraints = ret_expr.expr.constraints
    ...
    def _update_ret_variable(self, callsite, callee_function):
        ...
        """
        print("Update-ret in %s" % (callsite))
        killed_exprs = []
        new_backward_exprs = []
        get_ret_flag = False
    ...
    def _use_symbol_replace_real_ret_value(self, callsite, callee_addr):
        """
