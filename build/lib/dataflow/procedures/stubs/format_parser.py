...
from ...parse_ast import BVV, get_mem_permission
...
class FormatParser(SimProcedure):
    ...
    def _get_value(self, fmtstr_reg):
        live_defs = self.block.live_defs
        if fmtstr_reg in live_defs:
            fmt_at = live_defs[fmtstr_reg]
            print("fmt_action: %s" % (fmt_at))
            if type(fmt_at.value) is int:
                return fmt_at.value
    ...
    def _find_taint_propagate_in_format_v1(self, s, args, constrain=None):
        """
    ...
    def _parse(self, s, fmtstr_reg):
        ...
        fmtstr_ptr = self._get_value(fmtstr_reg)
