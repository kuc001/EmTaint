...
import logging
l = logging.getLogger("graph_method")
l.setLevel("INFO")
...
class ContainerNode(object):
    ...
    def __repr__(self):
        return 'ContainerNode[%s (%s)]' % (self._obj, self.index)
...
class GraphMethod():
    """
    Implements the common method in graph, mainly contain: dominator graph, post-dominator graph, CDG and so on.
    """

    def __init__(self, cfg, start, exit_blocks=None, no_construct=False):
        """
        Constructor.
        ...
        self._cfg = cfg
        self._start = start
        self._exit_blocks = list(exit_blocks) if exit_blocks else []
        ...
        if not no_construct:
            ...
            if isinstance(self._start, int):
                self._entry = self._cfg.get_node_by_addr(self._start)
            else:
                self._entry = self._start
    ...
    def get_node_pdom(self, block):
        # container_n = ContainerNode(block)
        stack = [block]
        post_doms = []

        while len(stack) > 0:
            n = stack.pop()
            pre_nodes = self._post_dom.predecessors(n)
            stack.extend(pre_nodes)
            post_doms.extend(pre_nodes)
        return post_doms

    def judge_post_dome(self, node, pdom_node):
        """
        :param node: a node in the CFG
        :param pdom_node: maybe pdom_node post dominator node
        :return True if pdom_node post dominator node
        """
        stack = [node]
        traversed_nodes = set()
        ...
        while len(stack) > 0:
            ...
            for pre_n in pre_nodes:
                if pdom_node == pre_n:
                    return True
                if pre_n not in traversed_nodes:
                    stack.append(pre_n)
                    traversed_nodes.add(pre_n)
        return False
    ...
    def _pd_normalize_graph(self):
        ...
        while len(queue) > 0:
            ...
            if type(node) is TemporaryNode:
                # This is for testing
                successors = [n for n in self._acyclic_cfg.graph.successors(node)]
        ...
        for exit_node in self._exit_blocks:
            container_node = container_nodes[exit_node]
            graph.add_edge(start_node, container_node)
