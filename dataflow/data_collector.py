...
class Collector(object):
        ...
        for addr, iptr_exprs in iptr_info.items():
            ...
            for iptr_expr in iptr_exprs:
                ...
                for new_iptr in new_iptrs:
                    tmp_datas = self.replace_load_value(new_iptr, stores_info, sim_action_info)
    ...
    def replace_one_load_value(self, data_ast, stores_info, sim_action_info):
        ...
        for child_ast in data_ast.recursive_children_asts:
            ...
            # elif ld_addr.op == 'BVV' and get_mem_permission(ld_addr.args[0]) in ['ro', 'rw']:
                ...
                read_values = self.read_recursive_data(ld_addr, read_type='data', end_flag=0, var_type=child_var_type)
                for value in read_values:
                    value_ast = BVV(value, child_ast.size())
    ...
    def replace_load_value(self, data_ast, stores_info, sim_action_info):
        """
        ...
        while data_worklist:
            data_ast = data_worklist.pop()
            tmp_asts = self.replace_one_load_value(data_ast, stores_info, sim_action_info)
            if tmp_asts:
                data_worklist.extend(tmp_asts)
    ...
    def guess_icall_targets(self, iptr_expr, stores_info, sim_action_info):
        ...
        print("simplify_load get: %s" % (tmp_datas))
